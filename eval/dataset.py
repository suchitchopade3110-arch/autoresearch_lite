...
def validate_custom_dataset(dir_path: str) -> Dict[str, str]:
    ...
    synthetic linear-boundary generator. Only checks the three files EXIST
    and that truth.json parses as JSON - row-level schema is the
    operator's own scorer's business (see eval/pipeline.py's scorer
    plugin), not this module's, so it isn't validated here.

    The security invariant that matters - truth.json never reaching the
    sandbox - is enforced by sandbox/executor.py mounting train.jsonl/
    test.jsonl by explicit name only, never the whole directory. That
    holds regardless of whether these three files were generated by
    generate_split() or dropped in by hand, so nothing here needs to
    special-case it.
    """
    train_path = os.path.join(dir_path, "train.jsonl")
    test_path = os.path.join(dir_path, "test.jsonl")
...
def resolve_dataset(dir_path: str, mode: str = "synthetic", **generate_split_kwargs) -> Dict[str, str]:
    ...
    paths - dispatches to the built-in synthetic generator (mode:
    "synthetic", the default, fully backward compatible) or to a
    validated bring-your-own dataset (mode: "custom"). generate_split_kwargs
    (n/seed/test_frac) are only meaningful for "synthetic" and are ignored
    for "custom".
    """
    if mode == "custom":
