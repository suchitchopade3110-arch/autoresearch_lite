...
def test_empty_diff_is_never_a_duplicate():
...
def test_no_prior_experiments_is_never_a_duplicate():
    with tempfile.TemporaryDirectory() as d:
        db = ExperimentDB(db_path=d)
        assert is_duplicate(DIFF_A, db) is False


def test_exact_diff_match_is_a_duplicate():
    with tempfile.TemporaryDirectory() as d:
        db = ExperimentDB(db_path=d)
...
def test_near_identical_diff_is_flagged_as_a_duplicate():
    """
...
def test_genuinely_different_diff_is_not_flagged():
    with tempfile.TemporaryDirectory() as d:
        db = ExperimentDB(db_path=d)
        db.store_experiment(hypothesis="goal", diff=DIFF_A, rationale="r", metrics={}, outcome="success")
...
def test_retrieve_experiments_includes_distance():
    with tempfile.TemporaryDirectory() as d:
        db = ExperimentDB(db_path=d)
        db.store_experiment(hypothesis="goal", diff=DIFF_A, rationale="r", metrics={}, outcome="success")
