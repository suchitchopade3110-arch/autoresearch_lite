...
def test_kpis_computed_from_actual_stored_data():
    with tempfile.TemporaryDirectory() as d:
        db = ExperimentDB(db_path=os.path.join(d, "chroma"))
        ...
        kpis = compute_kpis(db, store, evolution_report_path=os.path.join(d, "no_such_file.jsonl"))
...
def test_generate_report_file_matches_compute_kpis():
...
def test_duplicate_avoidance_rate_reads_from_evolution_report():
    with tempfile.TemporaryDirectory() as d:
        db = ExperimentDB(db_path=os.path.join(d, "chroma"))
        report_path = os.path.join(d, "evolution_report.jsonl")
