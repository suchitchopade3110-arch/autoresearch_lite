...
| 10 | Progressive-scaling stages ran on the honor system: `SUBSET_PERCENTAGE` was only an environment variable, and every stage mounted the SAME full `train.jsonl` regardless of it - a candidate that ignored the env var (or claimed a smaller subset than it actually used) could train on 100% of the data at the "1%" stage undetected. (`.eval_out` not being wiped between stages, the other half of this finding, was already fixed as a side effect of #1.) | `eval/dataset.py:write_subset` host-selects each stage's rows; `sandbox/executor.py:run_candidate` gained `train_path_override` to mount that file instead of the full one; wired through both orchestrator paths (`orchestrator/run.py`, `evolution/scheduler.py`'s `_run_eval_stages` via `EvolutionEngine.train_path`) | `tests/test_dataset.py::test_write_subset_writes_only_the_host_selected_rows`, `tests/test_sandbox.py::test_run_candidate_mounts_train_path_override_instead_of_dataset_dir_train`, `tests/test_evolution_scheduler.py::test_progressive_stages_mount_a_host_selected_subset_not_the_full_train_file` |
| 15 | `await_approval_decision`'s deadline branch ignored `decide()`'s return value - if a human's decision landed in the narrow window between the "still pending" read and the `decide("timed_out", ...)` call, the DB correctly held the human's decision but the function (and the orchestrator's outcome record) returned "timed\_out" regardless, disagreeing with the persisted record | `approval/gate.py:await_approval_decision` now re-reads and returns the real persisted status when `decide()` returns `False` (a no-op, meaning someone else already decided it) | `tests/test_approval_gate.py::test_decision_landing_exactly_at_the_deadline_returns_the_real_decision_not_timed_out` |
| 16 | Elite carryover always re-applied the previous generation's best diff verbatim against a fresh base, with no check that it hadn't already merged (in which case the diff was now a no-op or a hard apply failure against its own already-applied content) and no duplicate check (bypassing the same check ordinary candidate generation always runs) | `evolution/population.py:_carry_over_elite` skips a merged parent outright, duplicate-checks the diff, and dry-run-validates it against a fresh worktree before including it - falling back to ordinary generation for that slot if any of those fail | `tests/test_evolution_population.py::test_carry_over_elite_skips_a_merged_parent`, `test_carry_over_elite_skips_a_duplicate_of_memory`, `test_carry_over_elite_skips_a_diff_that_no_longer_applies_and_rolls_back`, `test_carry_over_elite_succeeds_for_a_valid_unmerged_parent` |

Finding #17 (several tests that pass without proving their claim, and
non-hermetic Chroma tests) is tracked but not fixed in this wave - the new
tests added throughout this wave already replace the pattern it flagged
(behavioral tests with real git/sandbox/scheduler state instead of source
greps) for every finding above; a dedicated pass over the REMAINING
